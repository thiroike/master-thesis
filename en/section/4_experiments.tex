...
On both datasets, the proposed method achieved performance surpassing all comparative methods.
...
Fig. \ref{fig:epsilon_comparison} illustrates the performance comparison between the proposed adaptive method and PolyDice-1 Loss with various fixed $\epsilon$ values.
On both datasets, the proposed method consistently outperforms all fixed $\epsilon$ settings, including the retrospectively optimized values.
...
Focal Loss resulted in lower performance than Dice Loss on both datasets. Note that while Focal Loss addresses class imbalance by reducing the loss of easy samples,
...
\begin{figure}[t]
    ...
    \caption{Comparison of segmentation performance (Dice coefficient) between the proposed adaptive method and PolyDice-1 Loss with various fixed $\epsilon$ values. The left graph shows the results for the CVC-ClinicDB dataset, and the right graph shows the results for the Kvasir-SEG dataset. The red dashed line represents the performance of the proposed method.}

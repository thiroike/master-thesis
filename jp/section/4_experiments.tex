...
提案法は全ての比較手法を上回る性能を達成した．
特に注目すべきは，テストデータに対して最適な固定$\epsilon$を事後探索したPolyDice-1 Loss (optimal) をも上回った点である．
図 \ref{fig:epsilon_comparison} に，提案法と，様々な固定 $\epsilon$ 値を用いた PolyDice-1 Loss との性能比較を示す．
両データセットにおいて，提案法は事後的に最適化された $\epsilon$ 値を含むすべての固定された設定を一貫して上回る性能を達成した．
...
Focal Loss は両データセットにおいて Dice Loss を下回る結果となった．

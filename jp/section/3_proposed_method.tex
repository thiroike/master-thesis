...
\begin{equation}
    \mathcal{P}_n' = {(i,j) \in \mathcal{P}_n \mid \mu_{\mathcal{P}_n} - 2\sigma_{\mathcal{P}_n} \leq I_{n, i, j} \leq \mu_{\mathcal{P}_n} + 2\sigma_{\mathcal{P}_n} }
\end{equation}
...
データセット全体のスコア集合$\{D_n\}_{n = 1} ^ N$に対し， $q$ パーセンタイル値を $D_q$，標準偏差を $\sigma_D$ とすると，正規化されたスコア $D_{n, \text{norm}}$ は次のように計算される．
...
$E_0 > 0$の場合，エポック $1$ から $E_0 - 1$として
$\epsilon=0$を固定したまま学習を行う．この期間により，モデルは不確実性推定に必要な基礎的な特徴表現を獲得する．
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$e = 1$ \textbf{to} $E$}
            % \State \Comment{\textbf{Phase 1: Difficulty Estimation \& Parameter Update}}
            \If{$e \geq E_0$ \textbf{and} $(e - E_0) \pmod \tau = 0$}
                \State Set model to evaluation mode (enable Dropout)
                
                % \State \textbf{Step 1: Uncertainty Quantification \& Aggregation}
                \For{$n = 1$ \textbf{to} $N$}
                    % \State Perform $T$ stochastic inferences (Eq. 8):
                    \State \quad $\hat{\mathbf{Y}}^{(t)} = f_{\mathbf{W}}(\mathbf{X}_n; \mathbf{z}^{(t)}), \quad \mathbf{z}^{(t)} \sim \text{Bernoulli}(1-p)$
                    % \State Calculate pixel-wise Mutual Information (Eq. 9):
                    \State \quad $I_{n, i,j} = H\left( \frac{1}{T} \sum_{t} \hat{y}_{n, i,j}^{(t)} \right) - \frac{1}{T} \sum_{t} H\left( \hat{y}_{n, i,j}^{(t)} \right)$
                    
                    % \State Outlier removal and score calculation (Eq. 11, 12):
            ...
            \State
            % \State \Comment{\textbf{Phase 2: Model Optimization (Minibatch Training)}}
